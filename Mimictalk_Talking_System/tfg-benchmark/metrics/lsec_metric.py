...
class LSECMetric:
    ...
    def calculate(self, audio_path, generated_video, video_processor, 
        ...
        try:
            ...
            if result['status'] == 'success':
                lsec_score = result['lsec']
                
                final_result = {
                    'name': 'LSE-C',
                    'value': float(lsec_score),
                    'offset': result.get('offset', 0.0),
                    'confidence': result.get('confidence', 0.0),
                    'lsed': result.get('lsed', 0.0),  # 也记录 LSE-D 值
                    'status': 'success',
                    'interpretation': self._interpret_score(lsec_score),
    ...
    def extract_features(self, audio_path, video_path):
        """
        提取音频和视频特征（用于调试）
        ...
        audio_feat = self.syncnet.extract_audio_features(audio_path)
        video_feat = self.syncnet.extract_video_features(video_path)
