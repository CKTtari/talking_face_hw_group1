...
- **页面布局**：
  - 左侧面板：当前步骤说明、任务进度显示和文件预览区域
  - 中间工作区：根据不同步骤显示相应的操作界面
  - 右侧控制面板：模型配置和参数设置
...
│   ├── chat.html              # 实时对话页面
│   └── workflow.html          # 一条龙体验页面
├── static/                     # 静态资源
│   ├── css/                   # CSS 样式文件
│   │   ├── style.css          # 全局样式（含新手引导样式）
│   │   ├── common.css         # 通用样式
│   │   ├── train.css          # 训练页面样式
│   │   ├── generate.css       # 生成页面样式
│   │   ├── chat.css           # 对话页面样式
│   │   └── workflow.css       # 一条龙体验页面样式
│   ├── js/                    # JavaScript 脚本
│   │   ├── ui.js              # 通用 UI 功能（含新手引导）
│   │   ├── train.js           # 训练页面脚本
│   │   ├── generate.js        # 生成页面脚本
│   │   └── chat.js            # 对话页面脚本
│   └── videos/                # 视频存储（示例）
...
- **样式框架**：自定义 CSS（响应式设计 + 主题系统）
- **数据传输**：JSON REST API
- **核心特性**：
  - CSS 变量驱动的主题系统
  - 原生 JavaScript 新手引导引擎
  - 响应式布局算法
